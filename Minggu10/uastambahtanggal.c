...
void writeLog(const char *feature) {
    FILE *logFile = fopen("log.txt", "a");  // Membuka file log dalam mode append
    if (logFile != NULL) {
        time_t t;
        struct tm *tm_info;
        char timeStr[20];  // Buffer untuk menyimpan string tanggal/waktu dalam format dd/mm/yyyy hh:mm

        time(&t);  // Mengambil waktu saat ini
        tm_info = localtime(&t);  // Mengonversi waktu ke waktu lokal

        // Format waktu menjadi dd/mm/yyyy hh:mm
        strftime(timeStr, sizeof(timeStr), "%d/%m/%Y %H:%M", tm_info);

        // Menulis log dengan format yang diinginkan
        fprintf(logFile, "[%s] Fitur '%s' dipanggil\n", timeStr, feature);
...
int main() {
    ...
    // Menambahkan log di awal aplikasi
    ...
    // Log berdasarkan pilihan menu
    if (pilihan == 1) {
        writeLog("Hitung Resistansi dipilih");
        int jumlahResistor;
        char tipeRangkaian;
        ...
        writeLog("Kalkulator Hukum Ohm dipilih");
        kalkulatorHukumOhm();
    } else if (pilihan == 3) {
        writeLog("Kalkulasi Pembagi Tegangan dipilih");
        kalkulasiPembagiTegangan();
    } else if (pilihan == 4) {
        writeLog("Cari Resistor Berdasarkan Nilai dipilih");
        double target;
